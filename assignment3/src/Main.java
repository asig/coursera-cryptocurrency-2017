...
public class Main {
	 ...
	 */
...

        {
            ...
            Block genesisBlock = new Block(null, pk_scrooge.getPublic());
            ...
            for (int i = 0; i < 20; i++) {
                ...
                spendCoinbaseTx.addOutput(Block.COINBASE, pk_scrooge.getPublic());
                spendCoinbaseTx.addInput(prevBlock.getCoinbase().getHash(), 0);
                spendCoinbaseTx.signTx(pk_scrooge.getPrivate(), 0);

                spendCoinbaseTx.finalize();
                blockHandler.processTx(spendCoinbaseTx);

                Block createdBlock = blockHandler.createBlock(pk_scrooge.getPublic());

                System.err.println("ITERATION " + i);
                System.err.println("createdBlock != null --> " + (createdBlock != null));
                System.err.println("createdBlock.getPrevBlockHash().equals(prevBlock.getHash()) --> " + (createdBlock.getPrevBlockHash().equals(prevBlock.getHash())));
                System.err.println("createdBlock.getTransaction(0).equals(spendCoinbaseTx) --> " + (createdBlock.getTransactions().size() == 1 && createdBlock.getTransaction(0).equals(spendCoinbaseTx)));
            ...
            System.err.println("passes = " + passes);
        }
